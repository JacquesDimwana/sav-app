...
export class IngredientsManagerPage implements OnInit {
  ...
  //ingredientSelectionne: { id: number; nom: string; sapo: number; ins: number; iode: number; volMousse: number; tenueMousse: number; douceur: number; lavant: number; durete: number; solubilite: number; sechage: number; estCorpsGras: boolean; };
  ...
  */
  ...
  // - saveIngredient(ingredient: Ingredient) -> Ajout ou met à jour
  //l'ingrédient passé en argument
  // - deleteIngredient(id: number) -> Surppime l'ingrédient de clé
  //primaire id
